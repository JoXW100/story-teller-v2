...
const getHealth = (metadata) => {
    switch (metadata.health?.type) {
        ...
        case CalculationMode.Modify:
            var dice = metadata.hitDice ? parseInt(metadata.hitDice) : 0;
            var mod = getAttributeModifier(metadata.con);
            var total = mod * metadata.level + (metadata.health.value ?? 0);
            return {
                value: Dice.average(dice, metadata.level) + total,
                element: <Elements.Roll options={{ dice: dice, num: metadata.level, mod: total, desc: "Health" }}/>
            }
        case CalculationMode.Auto:
        default:
            var dice = metadata.hitDice ? parseInt(metadata.hitDice) : 0;
            var mod = getAttributeModifier(metadata.con);
            var total = mod * metadata.level;
            return {
                value: Dice.average(dice, metadata.level) + total,
                element: <Elements.Roll options={{ dice: dice, num: metadata.level, mod: total, desc: "Health" }}/>
            }
        ...
        <>
            <Elements.Align>
                <Elements.Block>
                    ...
                    <div>
                        <b>Challenge </b>
                        { `${Math.floor(metadata.challenge) ? metadata.challenge : `1/${Math.floor(1/metadata.challenge)}`} (${metadata.xp ?? 0} XP)`}
                    </div>
                    <div>
